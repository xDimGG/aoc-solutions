...
REGION_SIZE = min(b[1] - b[0] for b in x_bounds) + 1
regions = []

for y in range(0, len(grid), REGION_SIZE):
	for x in range(0, len(grid[y]), REGION_SIZE):
		if grid[y][x] != ' ':
			regions.append([row[x:x+REGION_SIZE] for row in grid[y:y+REGION_SIZE]])

print(regions)

# Region: (Right, Down, Left, Up)
connections = {
	0: [5, 3, 2, 1],
	1: [2, 5, ]
}
